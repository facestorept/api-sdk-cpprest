...
{
    {
        ...
        for( auto& item : val[utility::conversions::to_string_t("data")].as_array() )
        {
            if(item.is_null())
            {
                m_Data.push_back( std::shared_ptr<Brand>(nullptr) );
            }
            else
            {
                std::shared_ptr<Brand> newItem(new Brand());
    ...
    if(val.has_field(utility::conversions::to_string_t("meta")))
    {
        ...
        {
            std::shared_ptr<MetaPartialResponse> newItem(new MetaPartialResponse());
...
std::vector<std::shared_ptr<Brand>>& Inline_response_200::getData()
{
    return m_Data;
}

void Inline_response_200::setData(std::vector<std::shared_ptr<Brand>> value)
{
    m_Data = value;
    m_DataIsSet = true;
}
...
std::shared_ptr<MetaPartialResponse> Inline_response_200::getMeta() const
{
    return m_Meta;
}


void Inline_response_200::setMeta(std::shared_ptr<MetaPartialResponse> value)
{
    m_Meta = value;
    m_MetaIsSet = true;
}
