...
web::json::value Inline_response_201_2::toJson() const
{
...
void Inline_response_201_2::fromJson(web::json::value& val)
{
    {
        m_Data.clear();
        std::vector<web::json::value> jsonArray;
        ...
        for( auto& item : val[utility::conversions::to_string_t("data")].as_array() )
        {
            if(item.is_null())
            {
                m_Data.push_back( std::shared_ptr<Tax>(nullptr) );
            }
            else
            {
                std::shared_ptr<Tax> newItem(new Tax());
...
void Inline_response_201_2::toMultipart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix) const
{
...
void Inline_response_201_2::fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix)
{
    utility::string_t namePrefix = prefix;
    if(namePrefix.size() > 0 && namePrefix.substr(namePrefix.size() - 1) != utility::conversions::to_string_t("."))
    {
...
std::vector<std::shared_ptr<Tax>>& Inline_response_201_2::getData()
{
    return m_Data;
}

void Inline_response_201_2::setData(std::vector<std::shared_ptr<Tax>> value)
{
    m_Data = value;
    m_DataIsSet = true;
}
bool Inline_response_201_2::dataIsSet() const
{
    return m_DataIsSet;
}

void Inline_response_201_2::unsetData()
{
    m_DataIsSet = false;
}
