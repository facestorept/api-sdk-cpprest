...
void Attribute_options::validate()
{
    // TODO: implement validation
}

web::json::value Attribute_options::toJson() const
{
...
void Attribute_options::fromJson(web::json::value& val)
{
    if(val.has_field(utility::conversions::to_string_t("data")))
    {
        if(!val[utility::conversions::to_string_t("data")].is_null())
        {
            std::shared_ptr<Options> newItem(new Options());
            newItem->fromJson(val[utility::conversions::to_string_t("data")]);
            setData( newItem );
        }
    }
}

void Attribute_options::toMultipart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix) const
{
...
void Attribute_options::fromMultiPart(std::shared_ptr<MultipartFormData> multipart, const utility::string_t& prefix)
{
    utility::string_t namePrefix = prefix;
    if(namePrefix.size() > 0 && namePrefix.substr(namePrefix.size() - 1) != utility::conversions::to_string_t("."))
    {
...
std::shared_ptr<Options> Attribute_options::getData() const
{
    return m_Data;
}


void Attribute_options::setData(std::shared_ptr<Options> value)
{
    m_Data = value;
    m_DataIsSet = true;
}
bool Attribute_options::dataIsSet() const
{
    return m_DataIsSet;
}

void Attribute_options::unsetData()
{
    m_DataIsSet = false;
}
